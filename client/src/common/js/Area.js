...
export default class Area {
  ...
  draw(data){
    ...
    this.xScale = d3.scaleBand()
      ...
      .domain(xData).range([0, width-right-left ])
    ...
    let area = this.svg.append('path')
      .attr('class','area-char')
    area.attr('d',areaPath(xData))
      .attr('stroke','pink')
      .attr('fill','pink')
      .attr('opacity','0.6')
    ...
    //y轴
    this.yScale = d3.scaleLinear().domain([0,maxY*1.2]).range([height - bottom - top,0])
    ...
    yAxisG.selectAll(".tick:not(:first-of-type) line").attr("stroke", "#777").attr("stroke-dasharray", "2,2");
    ...
    xAxisG.selectAll(".tick text").data(xData).text((d)=>{
      return d
    } )
